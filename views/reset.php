...
if(isset($_GET['email']) && isset($_GET['reset_token'])){
    ...
    $req = $db->prepare('SELECT * FROM user WHERE email = ? AND reset_token IS NOT NULL AND reset_token = ? ');
    $req->execute([$_GET['email'], $_GET['reset_token']]);
    $user = $req->fetch();
    if($user){
        if(!empty($_POST)){
            if(!empty($_POST['password']) && $_POST['password'] == $_POST['password_confirm']){
                $password = $_POST['password'];
                $db->prepare('UPDATE user SET password = ?, reset_token = NULL WHERE users."user".email = ?')->execute([$password  ,$_GET['email']]);
                session_start();
                    ...
                    <div class="col-md-6 col-sm-8 col-xs-8 w3_menu">
                        <div class="navigt">
                            <div class="col-md-6 col-sm-5 col-xs-5 top-nav-text">
                                <a class="page-scroll" href="#myModal2" data-toggle="modal" data-hover="LOGIN">Update Password</a>
                            </div>
                            ...
                            <div class="login-top sign-top">
                                <div class="agileits-login">
                                    ...
                                    <form action="" method="post">
                                        ...
                                        <input type="password" class="password" name="password_confirm" placeholder="password_confirm" required=""/>
